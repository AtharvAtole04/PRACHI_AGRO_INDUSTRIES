...
export default async function handler(req, res) {
  ...
  if (req.method !== 'GET') {
    try {
      const renderUrl = `https://prachi-agro-industries.onrender.com${req.url || '/api/categories'}`;
  ...
  try {
    ...
    console.error('Vercel MongoDB API Error (categories):', error);
    return res.status(500).json({ error: 'Failed to fetch categories from MongoDB', details: error.message });
  }
}
