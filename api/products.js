...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. GET Request (Get all products or single product by ID)
    if (req.method === 'GET') {
      if (productId) {
    ...
    // 2. POST Request (Create New Product)
    if (req.method === 'POST') {
    ...
    // 3. PUT Request (Update Product by ID)
    if (req.method === 'PUT') {
      if (!productId) {
    ...
    // 4. DELETE Request (Delete Product by ID)
