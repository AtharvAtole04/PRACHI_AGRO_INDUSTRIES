...
export default async function handler(req, res) {
  ...
  if (req.method !== 'GET') {
    try {
      const renderUrl = `https://prachi-agro-industries.onrender.com${req.url || '/api/blogs'}`;
  ...
  try {
    ...
    console.error('Vercel MongoDB API Error (blogs):', error);
    return res.status(500).json({ error: 'Failed to fetch blogs from MongoDB', details: error.message });
  }
}
