...
export default async function handler(req, res) {
  ...
  if (req.method !== 'GET') {
    try {
      const renderUrl = `https://prachi-agro-industries.onrender.com${req.url || '/api/reviews'}`;
  ...
  try {
    ...
    console.error('Vercel MongoDB API Error (reviews):', error);
    return res.status(500).json({ error: 'Failed to fetch reviews from MongoDB', details: error.message });
  }
}
