...
export default async function handler(req, res) {
  ...
  if (req.method !== 'GET') {
    try {
      const renderUrl = `https://prachi-agro-industries.onrender.com${req.url || '/api/crops'}`;
  ...
  try {
    ...
    const items = await db.collection('crops').find({}).toArray();
    ...
    console.error('Vercel MongoDB API Error (crops):', error);
    return res.status(500).json({ error: 'Failed to fetch crops from MongoDB', details: error.message });
  }
}
