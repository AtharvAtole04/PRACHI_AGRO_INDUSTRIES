...
import { ShoppingCart, Eye, Scale, Sparkles, Tag, Store } from 'lucide-react';
...
const ProductCard = ({ product, isInCompare, onCompare }) => {
  ...
  if (isDealer) {
    displayPrice = Math.round(selectedPack.price * (1 - dealerMargin / 100));
    memberBadge = (
      <span className="bg-pink-100 text-brand-magenta text-[9px] font-black px-1.5 py-0.5 rounded flex items-center gap-0.5">
        <Store size={10} />
        <span>डीलर होलसेल ({dealerMargin}%)</span>
      </span>
    );
  } else if (isFarmer) {
    displayPrice = Math.round(selectedPack.price * 0.95); // 5% farmer special discount
    memberBadge = (
      <span className="bg-emerald-100 text-brand-green-dark text-[9px] font-black px-1.5 py-0.5 rounded flex items-center gap-0.5">
        <Tag size={10} />
        <span>५% शेतकरी सूट</span>
      </span>
    );
  }
...
प्रमाण: ${quantity}
किंमत: ₹${displayPrice}
कृपया उपलब्धता व पेमेंट तपशील पाठवा.`;
  ...
  const handleAddToCart = () => {
    const customizedPack = {
      ...selectedPack,
      price: displayPrice
    };
    addToCart(product, customizedPack, quantity);
  };

  return (
    <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col h-full group">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {/* Pricing with Role Customization */}
          ...
          <div className="flex gap-2">
            ...
            {onCompare && (
              <button
                ...
                className={`px-3 rounded-lg flex items-center justify-center border transition-all duration-200 cursor-pointer ${
                  isInCompare 
